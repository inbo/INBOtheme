...
```{r {{theme}}-histogram-categorical, fig.cap = "geom_histogram() + scale_fill_discrete() with categorical data"}
diamonds$categorical <- factor(diamonds$cut, ordered = FALSE)
ggplot(diamonds, aes(x = price, fill = categorical)) +
  geom_histogram(binwidth = 1000)
```

```{r {{theme}}-histogram-oridinal, fig.cap = "geom_histogram() + scale_fill_discrete() with ordinal data"}
ggplot(diamonds, aes(x = price, fill = cut)) +
  geom_histogram(binwidth = 1000)
```

```{r {{theme}}-density-categorical, fig.cap = "geom_density() + scale_colour_discrete() with categorical data."}
ggplot(diamonds, aes(x = price, colour = categorical)) +
  geom_density()
```

```{r {{theme}}-density-ordinal, fig.cap = "geom_density() + scale_colour_discrete() with ordinal data."}
ggplot(diamonds, aes(x = price, colour = cut)) +
  geom_density()
```

```{r {{theme}}-lm, fig.cap = "geom_point() + geom_smooth() + scale_colour_discrete() + scale_fill_discrete()"}
ggplot(mtcars, aes(x = wt, y = mpg, colour = factor(cyl), fill = factor(cyl))) +
  geom_point() +
  geom_smooth(method = "lm")
```

```{r {{theme}}-lm2, fig.cap = "geom_point() + geom_smooth() + scale_colour_discrete() + scale_fill_discrete()"}
...
```{r {{theme}}-errorbar, fig.cap = "geom_point() + geom_errorbar() + scale_colour_discrete"}
ggplot(
  df,
