...
theme_INBO <- function (base_size = 12, base_family = "") {
  ...
  theme(
    ...
    text = element_text(family = base_family, face = "plain", colour = INBObrown, size = base_size, hjust = 0.5, vjust = 0.5, angle = 0, lineheight = 0.9, margin = margin(), debug = FALSE),

    axis.line = element_blank(),
    axis.text = element_text(size = rel(0.8), colour = INBOreddishbrown),
    axis.text.x = element_text(margin = margin(t = 0.8 * half_line / 2), vjust = 1, colour = INBOreddishbrown),
    axis.text.y = element_text(margin = margin(t = 0.8 * half_line / 2), hjust = 1, colour = INBOreddishbrown),
    axis.ticks = element_line(colour = INBOreddishbrown),
    axis.ticks.length = unit(0.15, "cm"),
    axis.title.x = element_text(margin = margin(r = 0.8 * half_line, l = 0.8 * half_line / 2), colour = INBObrown),
    axis.title.y = element_text(margin = margin(r = 0.8 * half_line, l = 0.8 * half_line / 2), angle = 90, colour = INBObrown),
    ...
    strip.text.x = element_text(margin = margin(t = half_line, b = half_line), colour = INBObrown),
    strip.text.y = element_text(margin = margin(t = half_line, b = half_line), angle = -90, colour = INBObrown),
